...
public class JiraAPI {
	...
	public String addingComment() {
		...
		RestAssured.baseURI = prop.getProperty("JiraHost");
		 ...
		 when().
		 post("/rest/api/2/issue/"+ReusableMethods.getSessionID()+"/comment").then().assertThat().statusCode(201).extract().response();
	...
	public void updatingComment() {
		...
		RestAssured.baseURI = prop.getProperty("JiraHost");
		 ...
		 when().
		 put("/rest/api/2/issue/"+ReusableMethods.getSessionID()+"/comment/"+addingComment()).then().assertThat().statusCode(200).extract().response();
